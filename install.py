...
def default_bin_dir() -> Path:
    """Where to drop the `agent-board` command so it lands on PATH."""
    return Path.home() / ("bin" if IS_WINDOWS else ".local/bin")


def install_cli(bin_dir: Path, name: str) -> None:
    """Install an `agent-board` command that runs cli.py via the venv."""
    py = venv_python()
    if not py.exists():
        print(f"  ! venv interpreter {py} not found; run without --skip-venv first. Skipping CLI.")
        return
    ...
    if IS_WINDOWS:
        ...
        target.write_text(
            ...
            "# Runs cli.py with the project's virtualenv, from anywhere.\n"
            "# Data lives in ~/.agent-board; override the backend with AGENT_BOARD_URL.\n"
            f'exec "{py}" "{cli_py}" "$@"\n')
        target.chmod(0o755)
    print(f"  Installed CLI command '{name}' -> {target}")

    if str(bin_dir) not in os.environ.get("PATH", "").split(os.pathsep):
        rc = "~/.zprofile" if os.environ.get("SHELL", "").endswith("zsh") else "~/.profile"
        print(f"  ! {bin_dir} is not on your PATH. Add it with:")
        print(f'      echo \'export PATH="{bin_dir}:$PATH"\' >> {rc} && source {rc}')
...
def main() -> None:
    ...
    ap.add_argument("--no-cli", action="store_true", help="Don't install the agent-board CLI command onto PATH.")
    ap.add_argument("--bin-dir", default=None,
                    help="Directory to install the CLI command into (default: ~/.local/bin).")
    ap.add_argument("--cli-name", default=DEFAULT_NAME, help=f"Name of the installed CLI command (default: {DEFAULT_NAME}).")
    ...
    if not args.no_cli:
        ...
        bin_dir = Path(args.bin_dir) if args.bin_dir else default_bin_dir()
        install_cli(bin_dir, args.cli_name)
