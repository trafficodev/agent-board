...
def _read_edges(board_id: str) -> list[Edge]:
...
def _write_edges(board_id: str, edges: list[Edge]) -> None:
    p = _edges_path(board_id)
...
def get_edge(board_id: str, edge_id: str) -> Edge | None:
    return next((e for e in _read_edges(board_id) if e.id == edge_id), None)
...
def delete_edges_for_card(board_id: str, card_id: str) -> None:
    """Called by card_store.delete_card so a deleted card never leaves
    dangling edges pointing at it — same cleanup pattern board_store's
    delete_column already applies to orphaned cards."""
    edges = _read_edges(board_id)
